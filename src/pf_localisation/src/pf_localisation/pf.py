...
class PFLocaliser(PFLocaliserBase):
    ...
    def choose(self,choice): #returns from the cumulative weight dictionary the resampled particle
        ...
        for key in self.weighted_dist:
            if key<choice:
                ...
                continue
            if key>=choice: #return particle if threshold reache
                if saved==None:
    ...
    def update_particle_cloud(self, scan):
        ...
        for particle in self.particlecloud.poses:
            ...
            new_PC=PoseArray()
            choice=0
            increment=cumulative_weight/self.NUMBER_PREDICTED_READINGS #choose increment for resamplin
    ...
    def estimate_pose(self):
        ...
        if (self.plotted==False):    #debugging data/ takes a snap shot of the system at the start and plots an arrow graph of headings and positions and marks the estimated spot
